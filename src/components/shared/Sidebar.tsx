...
import { IoLogOutOutline, IoHomeOutline, IoMailOutline, IoSettingsOutline } from "react-icons/io5";
...
const NAV_ITEMS: Item[] = [
  ...
  { label: "Courses",           href: "/dashboard/courses",  Icon: TbReport },
  { label: "Terms & Conditions",href: "/dashboard/terms",    Icon: IoMailOutline },
];
...
function NavItem({ item, active, onClick }: { item: Item; active: boolean; onClick?: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <Icon className="h-5 w-5 flex-shrink-0 transform transition-transform duration-200 group-hover:translate-x-1 group-hover:text-[#00000f]" />
...
const Sidebar = () => {
  ...
  return (
    <div className="flex">
      <button type="button" onClick={toggle} className="p-1 md:hidden z-50" aria-label="Toggle menu">
        {isOpen ? (
      ...
      >
        <div className="flex items-center justify-between px-4 py-2">
          ...
          <button type="button" onClick={toggle} className="p-2 md:hidden" aria-label="Close sidebar">
