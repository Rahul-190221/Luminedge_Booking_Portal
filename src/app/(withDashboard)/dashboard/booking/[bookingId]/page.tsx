...
const BookingId = ({ params }: { params: { bookingId: string } }) => {
  ...
  // Check if the server response contains the specific message
  const errorMessage = error.response?.data?.message || "An unexpected error occurred.";

  // Display a custom error message for already booked tests
  if (errorMessage.includes("already booked")) {
    toast.error("You already have booked a test for the selected date.");
  } else {
    toast.error(errorMessage); // Display the server-provided message or a fallback
  }
}
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-28 mb-8">
        ...
        <div className="w-full flex flex-col items-start">
  <label htmlFor="testSystem" className="block mb-2 font-medium">Test System</label>
  <select
    id="testSystem"
    className="select select-bordered bg-[#FACE39] text-black w-full"
    value={testSystem || ""}
    ...
    disabled={!isDropdownEnabled} // Disable if course is not IELTS
  >
    <option value="" disabled>Select Test System</option> {/* Placeholder */}
