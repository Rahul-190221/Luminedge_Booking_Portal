...
const BookingId = ({ params }: { params: { bookingId: string } }) => {
  ...
  const courseName =
    courses.find((c) => c._id === params.bookingId)?.name || "Unknown Course";
  ...
  const canProceed = useMemo(() => {
    if (isHome) {
      // Home → need date + manual time input
      return Boolean(selectedSlotId && value instanceof Date);
    }
    if (isCenter || isPaper) {
      // TC or Paper → need selected slot & schedule
      return Boolean(selectedSlotId && scheduleId);
    }
    return false;
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        // prefer the mock that matches the course; fallback to lastMock fields
        const matchingMock = mocks.find(
          (m: { mockType: string }) => m.mockType === courseName
        );

        if (matchingMock) {
          setUserMockType(matchingMock.mockType);
          setUserTestType(matchingMock.testType || null);
          setTestSystem(matchingMock.testSystem || "");
        } else if (data?.user?.mockType === courseName) {
          setUserMockType(data.user.mockType);
          setUserTestType(data.user.testType || null);
          setTestSystem(data.user.testSystem || "");
        } else {
          setUserMockType("N/A");
          setUserTestType(data?.user?.testType || "N/A");
          setTestSystem("");
        }
        ...
        // If the route isn't available locally, continue with safe defaults
  ...
  const handleProceed = async () => {
    // If a selection field is shown (IELTS), backend requires testSystem
    ...
    // Rescheduling guard: can only reschedule same course
    if (oldBookingId) {
      ...
      if (oldBooking.name !== courseName) {
        toast.error(
          `You can only reschedule your ${oldBooking.name} mock, not ${courseName}.`
        );
        return;
      }
    }
    ...
    // --- HOME booking (no scheduleId/slotId; backend expects bookingDate + testTime)
    if (isHome) {
      ...
      const selectedDateTime = new Date(
        `${value.toLocaleDateString("en-CA")}T${selectedSlotId}`
      );
      ...
      if (timeDifferenceInHours < 24) {
        toast.error(
          "Bookings must be made at least 24 hours in advance for home test."
        );
        return;
      }
      ...
      try {
        ...
        if (
          oldBookingId &&
          (errorMessage.includes("Insufficient mock tests") ||
          ...
          try {
            ...
            toast.error(
              secondError.response?.data?.message || "Error after retrying booking."
            );
          }
        } else {
    ...
    // --- TEST CENTER / PAPER booking (requires scheduleId & slotId)
      ...
      <div className="w-full max-w-5xl mx-auto px-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mb-2 w-full">
            ...
            {courseName === "IELTS" ? (
              (() => {
                ...
                if (uniqueTestTypes.length > 1) {
                  ...
                  // Read-only look, but uncontrolled (no React warning)
                  return (
                    <select
              ...
              // Non-IELTS: read-only, uncontrolled (no warning)
              <select
                className="select select-bordered bg-[#FACE39] text-[#00000f] w-full appearance-none pointer-events-none"
                defaultValue={userTestType || ""}
                aria-readonly="true"
          ...
          <div className="w-full flex flex-col items-start text-[#00000f]">
            ...
            {courseName === "IELTS" ? (
              ...
              // read-only, uncontrolled (no fake onChange)
              <select
                id="testSystem"
                className="select select-bordered bg-[#FACE39] text-black w-full appearance-none pointer-events-none"
                defaultValue={testSystem}
                aria-readonly="true"
              >
                <option value={testSystem}>{testSystem}</option>
              </select>
            ) : (
              // if nothing present but field is visible, allow choosing from course defaults (if any)
              <select
                id="testSystem"
                className="select select-bordered bg-[#FACE39] text-black w-full"
                value={testSystem}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 mb-2 w-full">
            ...
            {selectedLocation === "Home" && (
              <div className="w-full flex flex-col items-start">
                ...
                />
                {!selectedSlotId && (
                  <p className="text-red-500 text-base mt-1">Test Time is required.</p>
                )}
              </div>
            )}
          </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <h4 className="text-sm font-medium mt-2 text-[#00000f]">
                                Test Type:{" "}
                                <span className="font-bold text-[#00000f]">
                                  {schedule.testType}
                                </span>
                              </h4>
        ...
        <div className="w-full flex justify-end mt-6 mb-4">
          ...
          >
            ...
            {canProceed && (
              <span
                className="absolute inset-0 rounded-full ring-2 ring-[#FACE39] opacity-0 
